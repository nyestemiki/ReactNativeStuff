...
import { COLORS, dummyData, FONTS, SIZES, icons } from '../../../constants/milkTeaShop'
...
const Order = ({ navigation, route, appTheme }) => {
	...
	useEffect(() => {
		setMenu(dummyData.menuList.filter(({ category }) => category === selectedCategory))
	}, [selectedCategory])
	...
	const renderHeaderSection = () => {
		return (
			...
			>
				...
				>
					<IconButton icon={icons.leftArrow} onPress={() => navigation.goBack()} />
	...
	const renderTopTabBarSection = (number = 0) => {
		return (
			<View
				style={{
					flexDirection: 'row',
			...
			>
				...
				>
					<Text style={{ color: COLORS.white, ...FONTS.h3 }}>{number}</Text>
				</View>
			</View>
		)
	}
